...
// Automatically runs on Worker startup to ensure zero-config deployments.

export async function ensureDbInitialized(db: D1Database) {
  try {
    // Check if the 'accounts' table exists as a marker for initialization
    const check = await db.prepare(
      "SELECT name FROM sqlite_master WHERE type='table' AND name='accounts'"
    ).first()

    if (!check) {
      console.log('[DB] Database tables not found. Initializing schema...')
      ...
      const statements = [
        ...
        // 4. Full-Text Search Virtual Table
        `CREATE VIRTUAL TABLE IF NOT EXISTS emails_fts USING fts5(
            sender_name, sender_email, subject, body_text,
            content='emails', content_rowid='rowid'
        )`,

        // 5. Triggers for FTS (executed individually to avoid semicolon splitting issues)
      ...
      console.log('[DB] Database schema successfully auto-initialized.')
    }
  } catch (err: any) {
    console.error('[DB] Failed to auto-initialize database:', err.message, err.stack)
  }
}
